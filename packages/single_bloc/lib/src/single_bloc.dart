...
abstract class SingleBloc<E, D, A> extends Bloc<SingleBlocEvent, SingleBlocState<E, D>> {
  SingleBloc([SingleBlocState<E, D>? initialState]) : super(initialState ?? SingleBlocState()) {
    ...
    on<SingleBlocEvent>(
      (event, emit) async {
        ...
        try {
          ...
          late D data;
          if (result is Either) {
            result as Either<SingleBlocErrorInfo<E>, D>; // ensure that our either is with appropriate types
            if (result.isLeft()) {
          ...
          if (error is SingleBlocErrorInfo<E>) {
            state.error._value = error;
          } else {
            state.error._value = SingleBlocErrorInfo<E>(
